        ...
        if ($_FILES['image']['error'] == UPLOAD_ERR_OK) {
          ...
          if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
            ...
            if (mysqli_query($conn, $sql)) {
              echo "<strong class='text-zinc-800 text-4xl max-w-lg text-center'>O imóvel foi alterado com sucesso.</strong>";
            } else {
              echo "<strong class='text-zinc-800 text-4xl max-w-lg text-center'>Erro ao alterar o imóvel</strong>";
            }
          } else {
